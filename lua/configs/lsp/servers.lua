...
settings = {
            -- Volar/vtsls settings to enable takeovers and enhanced project awareness
            typescript = {
                inlayHints = { 
includeInlayParameterNameHints = 'all', 
            ...
            volar = {
                -- enable all features Volar provides
...
rust_analyzer = { root_dir = util_ok and util.root_pattern('Cargo.toml', '.git') or nil, settings = { ['rust-analyzer'] = {} } },
    gopls = { root_dir = util_ok and util.root_pattern('go.mod', 'go.work', '.git') ornil, settings = {} },
    ...
    -- Zig Language Server
    zls = {
        root_dir = util_okand util.root_pattern('build.zig', 'zig.mod', '.git') or nil,
        filetypes = { 'zig' },
    },
    tailwindcss = {
    ...
    eslint ={ on_attach = function(client, bufnr) client.server_capabilities.documentFormattingProvider = false client.server_capabilities.documentRangeFormattingProvider = false vim.keymap.set('n', '<leader>le', '<cmd>EslintFixAll<cr>', { buffer = bufnr, desc = 'ESLint Fix All' }) end, settings = {} },
