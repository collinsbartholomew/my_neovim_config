...
local function start_js_stack(base_caps, on_attach, servers)
    ...
    -- Prefer vtsls (Volar TypeScript Server) when available. Do NOT auto-start deprecated tsserver/ts_ls.
    local vtsls_cmd = executable_from_mason_or_path('vtsls') or executable_from_mason_or_path('vls')
    if vtsls_cmd then
        vim.lsp.start({ name = 'vtsls', cmd = { vtsls_cmd, '--stdio' }, root_dir = root, capabilities = base_caps, on_attach = on_attach, filetypes = { 'javascript','javascriptreact','typescript','typescriptreact' }, settings = servers and servers.vtsls and servers.vtsls.settings or nil })
    else
        -- Fall back to biome (already started) or warn if no TypeScript server found
        if not biome_cmd then
            vim.notify('No TypeScript server (vtsls) found. Consider installing vtsls via :Mason', vim.log.levels.WARN)
        end
    end

    -- Tailwind / web servers handled by start_web_stack (it will start tailwind/css/html as needed)
end
...
local function start_web_stack(base_caps, on_attach, servers)
    ...
    local function start_server_if(cmd_bin, name, opts)
        ...
        local config = vim.tbl_deep_extend('force', { name = name, cmd = { cmd, '--stdio' }, root_dir = root, capabilities = base_caps, on_attach = on_attach, filetypes = { ft } }, opts or {})
